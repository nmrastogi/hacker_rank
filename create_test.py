import requests

BASE_URL = "https://www.hackerrank.com/x/api/v3"

ACCESS_TOKEN      = "cb14cf0a7654375c50b8a6053cb84ad958f8f488e08a1b9291568ee0bb43d9cc"
JWT_ACCESS_TOKEN  = "eyJraWQiOiJkLTE3NTgxOTcxNjU3NjgiLCJ0eXAiOiJKV1QiLCJ2ZXJzaW9uIjoiNCIsImFsZyI6IlJTMjU2In0..YC6WEhLrdgsyjUg_wdAPB6wWiCQYCWr4tqdfxko__nBTdKxuNkBKiVHhGljy0A-GbOsEJCOgwcR3WC6IyV-XNX0rBMXb-ksXf85Amlme-JMLVtNMfS0AUWn6i20PzCFBfZEuM50HfsPMbahxdxvFhW9xcOKkvKodtpMCDTCuII-TPCotlIjeytARMSDse5d46eABOO5vth5Scu10cPTCZsy8K0pTsLdqHevtcDI0vGsUsJYxFV_pFx27I1yoaXY2-n1j6MN5XzePXDezl33j2DQr-1CO6Ayuf3B0A98eoB0JpQAnz9XP4711JoJgFtk01OeKWIF51Zmptt10xqdvQg"
JWT_REFRESH_TOKEN = ""
API_TOKEN         = "8262c4229d2834d93bf2f459e4df060bfc4c116a102a8163bb602aa5dc9ca64f"

TEST_B_ID = 2263157  # Your Test B


# ===========================================================
# SESSION
# ===========================================================

def make_session():
    session = requests.Session()
    session.headers.update({
        "Authorization": f"Bearer {ACCESS_TOKEN}",
        "X-Auth-Token": API_TOKEN,
        "x-jwt-token": JWT_ACCESS_TOKEN,
        "x-refresh-token": JWT_REFRESH_TOKEN,
        "Accept": "application/json",
        "Content-Type": "application/json"
    })
    return session


# ===========================================================
# INVITE CANDIDATE TO TEST B (100 percent correct)
# ===========================================================

def invite_candidate_to_test_b(email, name="Candidate"):
    session = make_session()

    url = f"{BASE_URL}/tests/{TEST_B_ID}/candidates"

    payload = {
        "email": email,
        "name": name,
        "send_email": True
    }

    print(f"Inviting {email} to Test B ({TEST_B_ID})...")
    response = session.post(url, json=payload)

    print("Status:", response.status_code)
    print("Response:", response.text)

    # HackerRank returns 200 even when successful
    if response.status_code == 200 and "status" in response.json() and response.json()["status"] is not False:
        print(f"\nSUCCESS: {email} has been invited.\n")
    elif response.status_code == 200:
        print("\nInvite request accepted. Check message for delivery status.\n")
    else:
        print("\nFAILED to invite candidate.\n")


# ===========================================================
# USAGE EXAMPLE
# ===========================================================

if __name__ == "__main__":
    invite_candidate_to_test_b("test@example.com", "Test User")